...
class Game:
    def __init__(self):
        ...
        values = Game.load_map(sys.argv[1] if len(sys.argv) > 1 else os.path.join(config.MAP_FOLDER, 'map1.txt'))
        ...
        class_ = getattr(module, sys.argv[2] if len(sys.argv) > 2 else 'Jocke')
        self.agent = class_(self.start[0], self.start[1],
                            f'{sys.argv[2]}.png' if len(sys.argv) > 2 else 'Jocke.png')
